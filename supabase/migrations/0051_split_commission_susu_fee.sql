-- The Account Summary daily report was combining two unrelated revenue
-- sources into one "Total Commission" column:
--   - withdrawal commission (a per-transaction fee charged on savings
--     withdrawals; susu is exempt)
--   - the susu day-31 company fee (a once-per-cycle amount, unrelated to
--     any withdrawal)
-- These are distinct revenue lines everywhere else in the app (Overview,
-- Finance, Transaction Summary all already show them as separate figures) —
-- this report was the one place still adding them together. Split
-- total_commission into total_withdrawal_commission and total_susu_fees.
...
begin
  ...
  select
    ...
    coalesce(susu_fee.total, 0) as total_susu_fees,
