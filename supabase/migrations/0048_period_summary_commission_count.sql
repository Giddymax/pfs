-- The Transaction Summary page shows a "N transactions" sub-hint under every
-- inflow/outflow row except "Commission paid" — compute_period_summary never
-- returned a count for it. Reusing withdrawal_count would be wrong: susu
-- withdrawals are commission-exempt, so not every withdrawal that counts
-- toward withdrawal_count actually charged a fee. Add a dedicated
-- commission_count = number of withdrawals in the period that actually
-- charged a fee (fee > 0), so the hint is accurate rather than borrowed.
