...
// Mirrors the Deposits report page's own search box exactly, so an admin
// exporting "what's currently on screen" gets the same rows they searched
// for — not the whole period's deposits.
...
export async function GET(request: Request) {
  ...
  let txns = ((data ?? []) as PeriodTransaction[]).filter((t) => t.type === "deposit");
  ...
  return xlsxResponse(rows, {
    ...
    colWidths: [12, 10, 24, 12, 16, 14, 14, 16, 18, 18, 16, 18, 18, 30],
  });
}
