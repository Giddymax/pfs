...
export async function POST(request: Request) {
  ...
  await notifyDeposit(supabase, data);
  return NextResponse.json({ transaction: data });
}

async function notifyDeposit(supabase: Awaited<ReturnType<typeof createClient>>, txn: Transaction) {
  ...
  const msg = smsTemplates.depositRecorded(client.full_name, txn.amount, txn.bal_after, account.account_number);

  if (shouldSendClientSms("deposit", client, settings)) {
