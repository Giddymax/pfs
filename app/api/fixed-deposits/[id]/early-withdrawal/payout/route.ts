...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase
    .rpc("process_early_withdrawal_payout", { p_fd_id: id, p_paid_by: user.id })
    .single<PayoutResult>();

  if (error) return NextResponse.json({ error: error.message }, { status: 400 });

  await notifyEarlyWithdrawalPaidOut(supabase, data);
  return NextResponse.json({ payout: data });
}

async function notifyEarlyWithdrawalPaidOut(supabase: Awaited<ReturnType<typeof createClient>>, payout: PayoutResult) {
  ...
  const msg = smsTemplates.fdEarlyWithdrawalPaidOut(payout.client_full_name, payout.amount);
