...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!profile || profile.role !== "admin") {
    return NextResponse.json({ error: "Only an admin can approve an emergency claim" }, { status: 403 });
  }

  const { data, error } = await supabase
    .rpc("approve_emergency_claim", { p_claim_id: id, p_approved_by: user.id })
    .single<SusuClaim>();

  if (error) return NextResponse.json({ error: error.message }, { status: 400 });

  await notifyClaimDecision(supabase, data, "approved");
...
async function notifyClaimDecision(
  ...
  await sendSms({
    to: client.phone,
    message: decision === "approved" ? smsTemplates.susuClaimApproved(client.full_name, claim.amount) : smsTemplates.susuClaimRejected(client.full_name),
    event: decision === "approved" ? "susu_claim_approved" : "susu_claim_rejected",
