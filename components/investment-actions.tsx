  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded-md bg-[#1F6E4A] px-3.5 py-2 text-[13px] font-semibold text-white transition-colors hover:bg-[#17583A]"
      >
        <Plus size={14} />
        Add investment
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#061B3A]/50 px-4 animate-fade-in">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            <div className="mb-5 flex items-center justify-between">
              <h3 className="text-[15px] font-semibold text-[#0033AA]">Record investment</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="mb-1.5 block text-[12.5px] font-medium text-[#0033AA]/75">Investment name *</label>
                <input
                  type="text"
                  required
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder="e.g. Short-term treasury placement"
              ...
              <div>
                <label className="mb-1.5 block text-[12.5px] font-medium text-[#0033AA]/75">Investment type *</label>
                <input
                  type="text"
                  required
                  value={investmentType}
                  onChange={(e) => setInvestmentType(e.target.value)}
                  placeholder="e.g. Treasury bill, shares, fixed income"
                  className="w-full rounded-md border border-[#0033AA]/15 bg-white px-3.5 py-2.5 text-[14px] text-[#0A2240] outline-none transition-colors focus:border-[#0062E1]"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1.5 block text-[12.5px] font-medium text-[#0033AA]/75">Amount invested (GHS) *</label>
                  <input
                    type="number"
                    required
                    min="0.01"
...
export function DeleteInvestmentButton({ id, title }: { id: string; title: string }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#061B3A]/50 px-4 animate-fade-in">
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            <div className="mb-3 flex items-start justify-between">
              <h3 className="text-[15px] font-semibold text-[#0033AA]">Delete investment?</h3>
            ...
            <p className="mb-5 text-[13.5px] leading-relaxed text-[#0A2240]/60">
              &ldquo;{title}&rdquo; will be permanently removed from the investment log.
            </p>
            {error && (
