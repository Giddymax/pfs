...
export function PrintWithdrawalsReportButton({
  transactions,
  from,
  to,
  accountBalance,
  ...
  transactions: WithdrawalRow[];
  from: string;
  to: string;
  accountBalance: number;
  totalWithdrawals: number;
  cashPaidToClients: number;
  totalFeesRetained: number;
            ...
            >
              ...
              <div className="flex items-start justify-between gap-6 pb-5">
                ...
                <div className="text-right text-[11px] text-[#0A2240]/45">
                  <p className="font-semibold text-[#0A2240]/60">Withdrawals Report</p>
              ...
              {/* KPI strip — mirrors the 8 stat cards shown on screen */}
              <div className="mb-6 grid grid-cols-4 gap-3">
                <SummaryBox label="Account Balance" value={formatGHS(accountBalance)} color="#0033AA" />
                <SummaryBox label="Total Withdrawals" value={formatGHS(totalWithdrawals)} color="#963522" />
                <SummaryBox label="Cash Paid to Clients" value={formatGHS(cashPaidToClients)} color="#B45309" />
                <SummaryBox label="Fees & Charges Retained" value={formatGHS(totalFeesRetained)} color="#6D28D9" />
                <SummaryBox label="Withdrawal Commission" value={formatGHS(withdrawalCommission)} color="#0369A1" />
                <SummaryBox label="Susu Fees" value={formatGHS(susuFees)} color="#0891B2" />
                <SummaryBox label="SMS Charge" value={formatGHS(smsCharge)} color="#BE185D" />
                <SummaryBox label="Processing Fee" value={formatGHS(processingFee)} color="#475569" />
              </div>

              <div className="mb-5">
                <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[#0A2240]/50">
                  Withdrawal Log ({transactions.length})
                </p>
                  ...
                  <table className="w-full text-left text-[11.5px]">
                    <thead>
                      <tr className="border-b border-[#0A2240]/10 bg-[#0A2240]/[0.04]">
                        ...
                        <th className="px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-[0.1em] text-[#0A2240]/50">Fee</th>
                      ...
                      {transactions.length === 0 ? (
                        ...
                        transactions.map((t) => (
                          <tr key={t.id} className={t.reversed_at ? "opacity-50" : ""}>
                            ...
                            <td className="whitespace-nowrap px-3 py-2 text-right tabular-nums font-medium text-[#963522]">
                              −{formatGHS(t.amount)}
                            </td>
                            <td className="whitespace-nowrap px-3 py-2 text-right tabular-nums text-[#0A2240]/55">
                              {t.fee > 0 ? formatGHS(t.fee) : "—"}
                            </td>
                    ...
                    {transactions.length > 0 && (
                      <tfoot>
                        <tr className="border-t-2 border-[#0A2240]/15 bg-[#0A2240]/[0.03]">
                          ...
                          <td className="px-3 py-2 text-right text-[11.5px] font-bold tabular-nums text-[#963522]">
                            {formatGHS(cashPaidToClients)}
                          </td>
                          <td className="px-3 py-2 text-right text-[11.5px] font-bold tabular-nums text-[#6D28D9]">
                            {formatGHS(totalFeesRetained)}
                          </td>
                          <td />
                        </tr>
                      </tfoot>
