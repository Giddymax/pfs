...
// ──────────────────────────────────────────────
// Change password (admin resets another account)
// ──────────────────────────────────────────────
...
export function ChangePasswordButton({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/staff/${profileId}/password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => setOpen(true)}
        className="inline-flex items-center gap-1 rounded px-2.5 py-1.5 text-[11.5px] font-medium text-[#7C3AED]/70 transition-colors hover:bg-[#7C3AED]/8 hover:text-[#7C3AED]"
      >
        <KeyRound size={12} />
        Password
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#061B3A]/50 px-4 animate-fade-in">
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            <div className="mb-5 flex items-start justify-between">
              <div>
                <h3 className="text-[15px] font-semibold text-[#0A2240]">Reset password</h3>
                <p className="mt-0.5 text-[12.5px] text-[#0A2240]/50">{fullName}</p>
              </div>
              ...
              <Field label="New password">
                <div className="relative">
                  <input
                    ...
                    minLength={8}
                    required
                    className="w-full rounded-md border border-[#0A2240]/15 px-3 py-2.5 pr-9 text-[13.5px] text-[#0A2240] outline-none transition-colors focus:border-[#0033AA] focus:ring-1 focus:ring-[#0033AA]/20"
                  />
                  <button
                    type="button"
                    onClick={() => setShowNew((v) => !v)}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#0A2240]/35 hover:text-[#0A2240]/70"
                    aria-label={showNew ? "Hide" : "Show"}
                  >
                    {showNew ? <EyeOff size={14} /> : <Eye size={14} />}
                  </button>
                </div>
                <p className="mt-1 text-[11px] text-[#0A2240]/40">Minimum 8 characters.</p>
              </Field>
              ...
              <div className="flex justify-end gap-2.5 pt-1">
                ...
                {!success && (
                  <button
                    type="submit"
                    disabled={loading}
                    className="inline-flex items-center gap-2 rounded-md bg-[#7C3AED] px-4 py-2 text-[13px] font-semibold text-white transition-colors hover:bg-[#6D28D9] disabled:opacity-60"
                  >
                    {loading && <Loader2 size={14} className="animate-spin" />}
